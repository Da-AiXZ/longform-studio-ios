...
长篇工坊是一个最低支持 iOS 16 的中文长篇小说创作工具。它采用 SwiftUI + UIKit，直接访问用户自己的 OpenAI Chat Completions 兼容 HTTPS 接口，不建设账号或云端服务。

这个项目把 AI 生成放在可审计的创作流程中：规划候选、章节候选、独立审稿、定向修订、人工批准、连续性事实入账。AI 结果不会直接覆盖当前正文，也不承诺作品一定签约、上架或产生收入。

## 已实现

- 作品库，新建向导，TXT/Markdown 导入，`.novelproj` 工程备份与恢复
- 写作、规划、资料、质检四个工作区
...
- `Sources/NovelCore`：跨平台数据模型、仓库、上下文预算、提示词、SSE、质检和差异算法
...
第一版只支持 OpenAI Chat Completions 的基本请求形状：
...
- `.novelproj` 包含规划、正文版本、审稿、事实和生成记录，但不包含 API Key。
- 自有样章只提取平均句长、对话比例、段长和视角等抽象指标，不保存原文。
- 诊断日志会删除认证 header 和 API Key，不上传任何服务器。
- iCloud/CloudKit、团队同步、账号计费和自动投稿不在第一版范围内。
